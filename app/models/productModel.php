...
class productModel extends DModel
{
    ...
    {
        $sql = "SELECT * FROM $tablePro  ORDER BY $tablePro.id_product DESC LIMIT $offset,6";
        return $this->db->select($sql);
    }
    ...
    public function listProductNew($tablePro)
    {
        $sql = "SELECT * FROM $tablePro ORDER BY $tablePro.id_product DESC LIMIT 7";
        return $this->db->select($sql);
    }
    ...
    public function quickViewProduct($tablePro, $id)
    {
        $sql = "SELECT * FROM $tablePro WHERE $tablePro.id_product='$id'";
        return $this->db->select($sql);
    }
    ...
    public function updateQuantity($tbl_pro, $data, $cond)
    {
        return $this->db->update($tbl_pro, $data, $cond);
    }
}
